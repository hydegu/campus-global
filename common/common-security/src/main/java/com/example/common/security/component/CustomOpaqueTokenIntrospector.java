...
public class CustomOpaqueTokenIntrospector implements OpaqueTokenIntrospector {
	...
	public OAuth2AuthenticatedPrincipal introspect(String token) {
		...
		try {
			...
			userDetails = optional.get().loadUserByUser((ExtraUser) tokenPrincipal);
		}
			...
			log.error("资源服务器 introspect Token 错误 {}", ex.getLocalizedMessage());
		}

		// 注入客户端信息，方便上下文中获取
		ExtraUser extraUser = (ExtraUser) userDetails;
		Objects.requireNonNull(extraUser)
			.getAttributes()
